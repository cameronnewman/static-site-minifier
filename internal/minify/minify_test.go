...
func TestJS(t *testing.T) {
	tests := []struct {
		...
		{"template with regex containing quote", "el.innerHTML = `<div foo=\"${raw.replace(/\"/g, \"&quot;\")}\">`;",
			"el.innerHTML=`<div foo=\"${raw.replace(/\"/g, \"&quot;\")}\">`;"},
		{"template with comment in expr", "let t = `${ a /* } */ + b }`;", "let t=`${ a /* } */ + b }`;"},
...
func TestHTML(t *testing.T) {
	tests := []struct {
		...
		{"self-closing raw tag with space", `<pre/ ><p>  x  </p>`, `<pre/><p> x </p>`},
		{"similar tag name is not raw", "<pretty>  a  b  </pretty>", "<pretty> a b </pretty>"},
		{"scriptype is not script", "<scriptype>  a  </scriptype>", "<scriptype> a </scriptype>"},
